...
4. **研究**：兩個子分頁：
   - **回測**：v17.0 策略歷史驗證，直接重用「戰報與個股」用的同一套 v17.0 進場/出場邏輯
     ...
     （v16.10 Alpha Backtest，用另一套跟戰報不一致的規則，已淘汰）——回答「這套策略的訊號
     真的可以信嗎」。
   - **因子健康度＋迴歸模型**：都是回答「哪個因子有效」這個問題，深度跟花費不同，合併在
     同一個子分頁裡，上半段先看免費/秒級的結果，決定要不要花 BigQuery 成本重新訓練下半段：
     - **因子健康度**（對應 Colab Cell 4）：單一因子跟未來 5 日報酬率的 Pearson 相關係數，
...
**目前的範圍**：`FactorModelHistory` 記錄跟顯示每次訓練結果，**不會**自動回頭改變 `Analysis.gs`
每天算 `Armor_Score` 用的權重（45/30/15/10 那組固定係數永遠不變）。但「要不要把這裡找到的
最佳權重接回每日選股邏輯」現在是可以自己選的——見下方「可切換的篩選邏輯版本」，不用改程式碼。
...
在「研究 → 因子健康度＋迴歸模型」子分頁把某一版迴歸結果標記「套用」之後：
...
- 每天算戰報時，`Analysis.gs` 的 `runAnalysis()` 會讀一次目前套用中的因子模型權重
  ...
  就不會顯示這行，不給可能誤導的部分預測值。這組欄位一律都會算，不管下面選的是哪一種篩選邏輯。
- 這組分數本身只是「多一個參考」，預設不影響選股。但「要不要讓它反過來影響選股」現在不用改
  程式碼，見下面「可切換的篩選邏輯版本」。

### 可切換的篩選邏輯版本

「戰報與個股」實際挑出哪些股票當「新進場訊號」（既有持股的止盈/止損判斷不受影響，固定不變），
可以在「研究 → 因子健康度＋迴歸模型」的「🎯 戰報篩選邏輯」卡片切換三種版本
（`Analysis.gs` 的 `SCREENING_STRATEGIES`，Script Property `SCREENING_STRATEGY` 存目前生效版本）：

- **`rule_v17`（現行，預設）**：原本的規則式門檻——法人參與度排名、成交量爆量排名、
  下跌接手率排名 + 趨勢分數，完全不依賴因子迴歸模型，不需要先套用任何模型。
- **`factor_model_rank`（因子模型排名精選）**：完全不看規則式門檻，改用套用中的抗跌力因子模型，
  對當天全部候選股票的「預測抗跌力」算橫斷面排名（`computePredictedResistanceRanks_`，跟
  `Armor_Score` 算法一樣是同一天內互相比較），取排名前 10% 當訊號。沒有套用抗跌力模型的話這個
  版本永遠是 0 檔訊號，`runAnalysis()` 會明確回報需要先去套用模型，不會安靜地顯示「沒有戰報」。
- **`hybrid`（規則式門檻＋模型排名混合）**：先過 `rule_v17` 的多頭排列 + 流動性門檻，再用模型
  排名做二次篩選（前 30%），兩邊都要通過才算訊號，一樣需要先套用抗跌力模型。

**先比較，再套用**：「研究 → 策略回測驗證」的表單有「篩選邏輯版本」下拉選單，可以用同一段歷史
區間分別對幾種版本各跑一次 `runBacktestV17_()`，比較勝率/平均報酬/最大回落之後，覺得某一版
表現比較好，再回到「🎯 戰報篩選邏輯」套用成戰報實際生效的版本——這是目前唯一支援的比較方式，
沒有自動網格搜尋最佳門檻參數（那需要額外的 out-of-sample 驗證機制，避免過度適配歷史資料，
現在刻意不做，先讓你自己比較幾個明確定義的策略版本）。
...
- **因子迴歸結果不會自動改變 `Armor_Score` 本身**：「套用」按鈕會在 `FactorModelHistory` 分頁
  做標記，`Armor_Score` 權重永遠是 `Analysis.gs` 裡寫死的 45/30/15/10，不會因為跑了迴歸就自動
  改變。但套用之後可以選擇要不要讓它影響「哪些股票算新進場訊號」（見上面「可切換的篩選邏輯
  版本」），這一步是你自己在後台選、不用改程式碼，也可以隨時切回 `rule_v17` 完全不受影響。

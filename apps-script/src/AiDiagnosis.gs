...
var AI_VERDICT_OPTIONS_ = ['強力買入', '分批布局', '觀望不追', '立刻退出', '體質轉強，加碼', '體質穩健，續抱', '體質轉弱，減碼', '體質惡化，出場'];
 ...
 * 查核流程（Goodinfo 查核／5年+TTM 財報／多流派辯證／CoVE 自我驗證），差別在於決策基準——
 * 使用者明確表示可以長期持有、能承受短期股價波動，所以這裡刻意不是「損益管理」視角（原本
 * 用「持股成本／持有天數／目前損益%」當主要決策依據，四個分類也是損益導向：分批獲利入袋、
 * 觸發止損平倉…），而是「體質評估」視角：判斷基準是這家公司的基本面（財報/護城河/籌碼
 * 趨勢）從進場到現在有沒有實質變化，不是帳面賺賠多少。持股成本/損益% 還是會傳給 AI（見
 * buildDiagnosisPrompt_），但只當背景參考（部位規模/風險曝險程度），明確要求不能拿來當
 * 決策的主要依據，避免使用者被短期價格波動綁架、錯過真正該長抱的體質健康股票。
 */
 ...
 * 深度查核流程，但強制注入使用者實際持有這檔股票的成本/持有天數/損益%（僅供背景參考），
 * 改用持股專用、以體質變化為準的決策分類（加碼/續抱/減碼/出場），回答的是「這筆部位的
 * 基本面撐不撐得住我繼續抱」，不是「要不要進場」，也不是「現在該不該獲利了結/停損」。
 * code 必須是目前「持有中」的股票，否則沒有成本/損益資訊可以注入。
 */
